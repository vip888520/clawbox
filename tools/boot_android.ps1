...
$args = @(
  ...
  '-drive', "if=none,id=system,file=$img\system.img,format=raw",
  '-device', 'virtio-blk-device,drive=system',
  '-drive', "if=none,id=vendor,file=$img\vendor.img,format=raw",
  '-device', 'virtio-blk-device,drive=vendor',
  '-drive', "if=none,id=data,file=$avd\userdata-qemu.img,format=raw",
  '-device', 'virtio-blk-device,drive=data',
  '-netdev', 'user,id=mynet,hostfwd=tcp::15555-:5555',
  '-device', 'virtio-net-device,netdev=mynet',
  '-append', 'console=ttyAMA0 androidboot.hardware=ranchu qemu=1 androidboot.selinux=permissive',
  '-serial', "file:$serial",
  '-no-reboot'
)
Start-Process -FilePath "$q\qemu-system-aarch64.exe" -ArgumentList $args -WindowStyle Hidden
Write-Host 'Android arm64 booting (TCG, serial to file)'
