...
$dir = 'C:\Temp\clawbox-arm25'
$env:PATH = "$q;" + $env:PATH
$env:QEMU_AUDIO_DRV = 'none'
$serial = 'C:\Temp\clawbox-arm25\serial.log'
if (Test-Path $serial) { Remove-Item $serial }
Set-Location $dir
& "$q\qemu-system-aarch64.exe" -M virt -cpu cortex-a53 -smp 1 -m 2048 `
  -kernel "$dir\kernel-ranchu" -initrd "$dir\ramdisk.img" `
  -drive "if=none,id=system,file=$dir\system.qcow2,format=qcow2" -device virtio-blk-device,drive=system `
  ...
  -serial "file:$serial" -no-reboot 2>&1 | Tee-Object -FilePath (Join-Path $ws 'qemu25-console.log')
